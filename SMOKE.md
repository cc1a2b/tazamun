...
- **Windows (self-hosted): green** (commit `bb474f5`), after two root-cause fixes:
  1. **Stale runner cache.** The first runs hit `error[E0463]: can't find crate
     for tazamun` across downstream targets with no `(lib)` source error, while
     the identical commit passed on Linux — a killed/stale artifact on the
     runner's persistent 7.2 GB `target/` cache (disk was fine). Hardened at the
     root with a `cargo clean -p tazamun` CI guard so the class of failure cannot
     recur; the clean build then compiled and ran the suite.
  2. **Multi-homed telemetry flake.** With the build fixed, one real failure
     surfaced: `health::telemetry_snapshot_after_mesh_is_direct_and_sane`
     asserted the loopback link settles to grade `Good`, but the Windows runner
     host is pathologically multi-homed (Hyper-V + WSL vSwitches + physical
     NICs), so QUIC keeps migrating the path and the flaps hold it at `Fair`.
     The test now accepts a healthy settled grade (Good **or** Fair) — a
     degraded link still grades Poor/Offline and fails. Product grading logic is
     unchanged (unit-tested in `net::telemetry`).

  Windows-specific product behavior (long paths, sharing-violation retry,
  non-portable→unapplied, Scheduled-Task service) stays covered by the Windows
  integration tests + the P4/P5 Windows SMOKE records above; a fresh native
  interactive ladder still wants a hands-on Windows session.
