...
/// The monochrome brand mark (drawn with `currentColor`, no inline styles or
/// scripts), inlined into the dashboard header — no external asset, CSP intact.
const LOGO_MARK: &str = include_str!("../assets/branding/tazamun-mono.svg");
...
async fn route(server: &Server, req: &Request) -> Vec<u8> {
    ...
    match (req.method.as_str(), req.path.as_str()) {
        ...
        // P18 conflict center: two thin verbs; the browser composes them with
        // lock/unlock exactly like the CLI does.
        ("POST", "/api/conflict/apply") => {
            guarded(server, req, parse_conflict_apply(&req.body)).await
        }
...
fn parse_conflict_apply(body: &[u8]) -> ParsedReq {
    let v: serde_json::Value = serde_json::from_slice(body).map_err(|e| e.to_string())?;
    let id = v
        .get("id")
        .and_then(|x| x.as_str())
    ...
    let target = v
        ...
        .to_string();
    Ok(IpcRequest::ConflictApply { id, target })
}

fn parse_conflict_discard(body: &[u8]) -> ParsedReq {
    let v: serde_json::Value = serde_json::from_slice(body).map_err(|e| e.to_string())?;
    let id = v
        .get("id")
        .and_then(|x| x.as_str())
        .ok_or("missing string field `id`")?
        .to_string();
    Ok(IpcRequest::ConflictDiscard { id })
}
...
mod tests {
    ...
    fn index_sets_csp_nonce_and_no_frame() {
        ...
        // The placeholders are fully substituted, and the brand mark (inline
        // SVG, no inline style/script) is injected into the header.
        assert!(!text.contains("__NONCE__"));
        assert!(
            !text.contains("__LOGO__"),
            "logo placeholder must be substituted"
        );
        assert!(
            text.contains("<svg"),
            "brand mark must be inlined into the page"
        );
    }
