...
pub async fn build_endpoint(secret_key: SecretKey, cfg: &NetConfig) -> Result<Endpoint, NetError> {
    build_endpoint_with_alpns(
        secret_key,
        cfg,
        vec![CTL_ALPN.to_vec(), iroh_blobs::ALPN.to_vec()],
    )
    .await
}

/// Builds an endpoint advertising an explicit ALPN set. The session daemon
/// uses [`build_endpoint`] (control + blobs); the one-shot `send`/`receive`
/// (P12) uses this with just its own ALPN, sharing all the NAT/relay/LAN
/// wiring without pulling in the session protocols.
pub async fn build_endpoint_with_alpns(
