...
## Branding vs. signing — what the embedded identity does and does NOT do

As of the branding pass the Windows binary carries an **embedded icon** and a
**VERSIONINFO** resource (via `winresource` in `build.rs`, Windows-target only,
build-dependency, no runtime cost). That populates **Properties → Details**:

| Field | Value |
| --- | --- |
| File description | tazamun — strict-checkout P2P folder sync |
| Product name | tazamun |
| Company name | @CC1A2B |
| Copyright | © 2026 @CC1A2B |
| Original filename | tazamun.exe |
| File / Product version | `CARGO_PKG_VERSION` + git short hash |

**This is metadata text only.** It does **not** set the **"Publisher"** shown in
the UAC / SmartScreen prompt, and it does **not** remove the SmartScreen /
Defender warning — those come **only** from an Authenticode signature with a real
CA certificate (below). After signing, the certificate's subject becomes the
Authenticode publisher; with an EV cert the SmartScreen warning clears
immediately, with an OV cert as download reputation accrues. Until then the
Details tab reads `@CC1A2B` but the prompt still says **"Unknown publisher."**
The binary is signing-**ready**; the signature is the post-v0.1 purchase.

The multi-resolution icon (`assets/branding/tazamun.ico`, 16–256) shows in
Explorer, the taskbar, and Alt-Tab **today**, with no certificate. The macOS
`.icns` (`assets/branding/tazamun.icns`) is generated for the app bundle; its
Developer ID signing + notarization path is below.
