...
- **Merge to `main` only when** the three gates are green **and** the clean-repo
  check passes (see below). Record the decision in `DECISIONS.md` in the merge
  commit.
- **No tags at phase boundaries.** `Cargo.toml` holds the *last released*
  version during a milestone's development — the version is not a per-phase
  marker. It advances only when a milestone ships: `0.1.0` shipped the v0.1 line,
  `0.2.0` the v0.2 line (P10–P20). The annotated `vX.Y.Z` tag is created after the
  milestone's final phase passes, and that tag is what triggers `release.yml`.
