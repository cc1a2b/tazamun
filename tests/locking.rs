...
/// P10: a `viewer`-role folder is refused every local edit path with the
/// role error — checked BEFORE reachability, so the answer is the same
/// offline (this single node has zero peers) as online.
#[tokio::test(flavor = "multi_thread")]
async fn viewer_role_refuses_local_edits_before_reachability() {
    let dir = TestNode::init_dir();
    std::fs::write(dir.path().join("doc.txt"), b"seed").unwrap();
    let mut st = tazamun::state::AppState::load(dir.path()).unwrap();
    st.config.role = tazamun::state::NodeRole::Viewer;
    st.save(dir.path()).unwrap();
    let node = TestNode::start(dir).await;

    // Lock: role_forbidden, not strict_offline — the role gate answers first.
    let denied = node.lock("doc.txt").await;
    assert!(!denied.ok);
    assert_eq!(
        denied.error.as_ref().map(|e| e.code.as_str()),
        Some("role_forbidden"),
    ...
    // Unlock and restore take the same refusal.
    let denied = node.unlock("doc.txt").await;
    assert_eq!(
        denied.error.as_ref().map(|e| e.code.as_str()),
        Some("role_forbidden"),
        "{denied:?}"
